...
using glm::vec3;
void UnitDisk::build(void* data){
    const int SEGMENT_SUBDIV = 6;
    //Initialize buffers on GPU
    glGenBuffers(1, &vertex_buffer);
    glGenBuffers(1, &index_buffer);

    float d_angle = 2 * M_PI / (6 * SEGMENT_SUBDIV);
    const float D30 = M_PI / 6;
    float angle = 0;
    vec3 c1, c2;
    float Z = THICKNESS;
    for (int s = 0; s < 6; s++) {
        ...
        c2 = vec3{OUTER_RAD * cos(angle + D30),
                OUTER_RAD * sin(angle + D30), Z};
...
void UnitDisk::render(bool outline) const {
    /* bind vertex buffer */
    glBindBuffer(GL_ARRAY_BUFFER, vertex_buffer);
